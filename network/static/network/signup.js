        ...
        element.addEventListener("change", event => {
            if(event.target.files[0]) {
                ...
                if(event.target.id == 'profile') {
                    event.target.parentElement.querySelector('.custom-file-label').innerHTML = `<span style="color: #1da1f2;">Choose profile picture<span>`;
                }
                else if(event.target.id == 'cover') {
                    event.target.parentElement.querySelector('.custom-file-label').innerHTML = `<span style="color: #1da1f2;">Choose cover picture<span>`;
                }
            }
        });
    });
